...
# test<- readRDS(paste0(directory,"SimulationResults_Seed",seed,".RDS"))
```
...
```
```{r}
HistoryTable = cbind(SimResults1$SelectedObservationHistory,
                     SimResults2$SelectedObservationHistory) 
colnames(HistoryTable) = c("Naive", "Rashomon")
HistoryTable
```
```{r}
plot(SimResults1$DeltaMetricVec, type = "l")
plot(SimResults2$DeltaMetricVec, type = "l")
```
